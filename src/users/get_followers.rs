use crate::{
    types::user::followers::{Follower, FollowerRoot},
    Farcaster,
};
...
impl Farcaster {
    ...
    pub async fn get_followers(
        ...
        username: Option<String>,
        address: Option<String>,
    ) -> Result<Vec<Follower>, Box<dyn std::error::Error>> {
        ...
        let follower_request = reqwest::get(format!(
            "https://api.farcaster.xyz/indexer/followers/{}",
            res_address
        ))
        .await?
        .text()
        ...
        let followers: Result<FollowerRoot, serde_json::Error> =
            serde_json::from_str(&follower_request);
