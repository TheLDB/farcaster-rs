...
impl Farcaster {
    ...
    pub async fn get_notifications(&self, username: Option<String>, address: Option<String>, page: i64) -> Result<NotifRoot, Box<dyn std::error::Error>> {
        ...
        if username.is_some() {
            let profile = self.get_user_by_username(username.unwrap()).await.unwrap();
        ...
        let req = reqwest::get(request_address).await.unwrap().text().await.unwrap();
        let notifications: NotifRoot = serde_json::from_str(&req).unwrap();

        if page > 0 {
            let mut loopy = 0;
            let mut new_notifications: String = "".to_string();
            while loopy != page {
                new_notifications = reqwest::get(notifications.meta.next.clone()).await.unwrap().text().await.unwrap();
                loopy += 1;
            }

            let notifications: NotifRoot = serde_json::from_str(&new_notifications).unwrap();
